...
import static Project.Librarian.*;
...
public class Reader extends Person implements User {
    ...
    //ABSTRACTION
    // abstract class will become imp only when it has implementation classes
    @Override
    ...
    public void rentBook() {
        ...
        if (this.searchUser(readerId) != -1 && this.searchBook(bookId) != -1) {
            if (reader_list.get(readerId).isIsBlock() && book_list.get(bookId).getReader_id() != 0) {
                System.out.println("Oops! Reader is Blocked or Book already Rented.");
            } else {
                ...
                if (this.checkReturnDate(d, m, y)) {
                    System.out.println("It is a Week ago, Neither Block nor Fine");
                } else {
                    System.out.println("\nExceeded a week! You are Blocked with Fine 10%");
                }
